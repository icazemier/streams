...
class BackToBasic extends BaseExample {
    async example(): Promise<void> {
        ...
        const streamToFilePath = join(Files.TEMP_DIR, '07-back-to-basic.txt');
        ...
        readable.on('data', (/*chunk*/) => {
            logger.debug(`${++readableStep} A readable "data" event received`);
        });
        readable.on('error', (error) => {
            logger.error(error);
        });
        readable.on('end', () => {
            logger.debug(`${++readableStep} A readable "end" event received`);
        });
        readable.on('close', () => {
            logger.debug(`${++readableStep} A readable "close" event received`);
        });
        readable.on('pause', () => {
            logger.debug(`${++readableStep} A readable "pause" event received`);
        });
